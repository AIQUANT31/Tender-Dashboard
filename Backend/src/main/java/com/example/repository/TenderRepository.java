...
public interface TenderRepository extends JpaRepository<Tender, Long> {
    ...
    List<Tender> findByUserTypeOrderByCreatedAtDesc(String userType);
    
    @Query("SELECT t FROM Tender t WHERE UPPER(t.userType) = UPPER(:userType) ORDER BY t.createdAt DESC")
    List<Tender> findByUserType(@Param("userType") String userType);
    
    @Query("SELECT t FROM Tender t WHERE t.userType IS NULL OR UPPER(t.userType) = UPPER(:userType) ORDER BY t.createdAt DESC")
    List<Tender> findByUserTypeIncludingNull(@Param("userType") String userType);
}
