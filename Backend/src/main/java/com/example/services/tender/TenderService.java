...
public class TenderService {
    ...
    public List<Tender> getTendersByUserType(String userType) {
        logger.info("Fetching tenders by userType: {} (Cache Miss - loading from DB)", userType);
        List<Tender> tenders;
        
        if (userType != null && !userType.trim().isEmpty()) {
            tenders = tenderRepository.findByUserTypeIncludingNull(userType);
        } else {
            // If no userType specified, return all tenders
            tenders = tenderRepository.findAllTendersSorted();
        }
        
        updateTenderStatusesBasedOnDeadline(tenders);
        
        logger.info("Found {} tenders for userType: {}", tenders.size(), userType);
        return tenders;
    }
