        ...
        try {
            ...
            try (ZipOutputStream zos = new ZipOutputStream(new BufferedOutputStream(Files.newOutputStream(zipPath)))) {
                ...
                for (String filePath : filePaths) {
                    ...
                    if (originalFileName.matches("^bid_\\d+_\\d+.*")) {
                        
                        standardFileName = originalFileName;
                    } else {
                       
                        String zipTimestamp = new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());
                        standardFileName = prefix + "_" + zipTimestamp + "_" + String.format("%04d", fileIndex) + fileExtension;
                    }
                    fileIndex++;
                    
                     Path sourcePath = Paths.get(UPLOAD_DIR, originalFileName);
                    
                    if (Files.exists(sourcePath) && Files.isReadable(sourcePath)) {
                        logger.info("Adding file to ZIP: {} (size: {})", sourcePath, Files.size(sourcePath));
                        ZipEntry zipEntry = new ZipEntry(standardFileName);
                        zos.putNextEntry(zipEntry);
                        ...
                        filesAdded++;
                        logger.debug("Added to ZIP: {}", standardFileName);
                    } else {
                        logger.warn("File not found or not readable: {}", filePath);
                    }
                }
                
                logger.info("Total files added to ZIP: {}", filesAdded);
            }
            ...
            if (Files.exists(zipPath) && Files.size(zipPath) > 0) {
                logger.info("ZIP file verified: {} (size: {})", zipFileName, Files.size(zipPath));
            } else {
                logger.warn("ZIP file was created but appears to be empty");
            }
