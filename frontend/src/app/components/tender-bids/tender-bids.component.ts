...
export class TenderBidsComponent implements OnInit {
  ...
  loadBids() {
    ...
    this.http.get<BidWithBidder[]>(`http://localhost:8080/api/bids/tender/${this.tender.id}/with-bidders`).subscribe({
      next: (bids) => {
        ...
        if (bids.length > 0) {
          console.log('First bid documentPath:', bids[0].documentPath);
          console.log('First bid zipFilePath:', bids[0].zipFilePath);
        }
  ...
  downloadZip(bidId: number, zipPath: string): void {
    if (bidId && zipPath) {
